...
namespace ConsoleApp1
{
    ...
    class Character : ICompare
    {
        ...
        void ICompare.CompareRace(ref object a)
        {
            Character B = (Character)a;
            if (race == B.race)
            {
                Console.WriteLine("Расы {0} и {1} совпадают\n", name, B.name);
            }
            else
            {
                Console.WriteLine("Расы {0} и {1} не совпадают\n", name, B.name);
            }
        }
        public int Age
        {
    ...
    class Program : Mage, ILVL
    {
        static void Main(string[] args)
        {
            ...
            Mage FourthObject = new Mage(250, Race.Elf, "Avallakh", Gender.Male, 70, 100, 1);
            Necromant FifthObject = new Necromant(100, Race.Ork, "Guldan", Gender.Male, 80, 150, 60, 100, 1);
            var characters = new List<Mage>
            {
               FourthObject, FifthObject
            };

            void ShowAge(Mage mage)
            {
                Console.WriteLine('1');
            }

            ShowAge(FourthObject);
            ShowAge(FifthObject);
...
//Добавить интерфейсы для чего либо.
